...
The nightly Jenkins job (`Jenkinsfile_nightly`) supports **aat** or **preview** (`ENVIRONMENT` / `PR_NUMBER`), `PLAYWRIGHT_GREP_TAG` (including `@nightly`, `@smoke`, `@e2e`, `@crossbrowser`, `@regression`, `@PR`, `@enforcement`, `@commoncomp`, `(all tests)` for no tag filter), and `PLAYWRIGHT_SPEC` → `E2E_SPEC` (comma- or semicolon-separated path keywords). Filtering is applied in `src/e2eTest/playwright.config.ts` (aligned with [pcs-frontend HDPI-6105 / configurable E2E](https://github.com/hmcts/pcs-frontend/pull/1132)). PR builds (`Jenkinsfile_CNP`) set `E2E_SUITE` / `E2E_TEST_SCOPE` / `E2E_SPEC` from `enable_e2e_*` labels and optional `e2e-tag:` / `e2e-spec:` labels. CI runs E2E via `src/e2eTest/bin/run-e2e-suite.sh` (invoked by `./gradlew runE2eTests`).
