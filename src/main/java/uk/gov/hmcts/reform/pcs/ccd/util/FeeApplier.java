...
public class FeeApplier {
    ...
    public <T> void applyFeeAmount(T caseData, FeeType feeType, BiConsumer<T, String> setter) {
        try {
            BigDecimal feeAmount = feeService.getFee(feeType).getFeeAmount();
            String formatted = feeFormatter.formatFee(feeAmount);
            setter.accept(caseData, formatted != null ? formatted : UNABLE_TO_RETRIEVE);
        } catch (Exception e) {
            log.error("Error while getting {} fee", feeType.name(), e);
            setter.accept(caseData, UNABLE_TO_RETRIEVE);
        }
    }
