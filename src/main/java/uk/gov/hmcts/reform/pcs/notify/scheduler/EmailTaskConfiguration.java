package uk.gov.hmcts.reform.pcs.notify.scheduler;
...
public class EmailTaskConfiguration {
    ...
    public CustomTask<EmailState> sendEmailTask() {
        return Tasks.custom(sendEmailTask).execute((
            ...
            try {
                // Convert to EmailNotificationRequest using your existing model
                ...
                // Use your existing NotificationService to send the email
                ...
                // Chain to verification task with 30-second delay to allow GOV.UK Notify to process
    ...
    public CustomTask<EmailState> verifyEmailTask() {
        return Tasks.custom(verifyEmailTask)
            .execute((taskInstance, executionContext) -> {
                EmailState emailState = taskInstance.getData();
                log.info("The time is {}, Verifying email delivery for notification ID: {}", Instant.now(), emailState.notificationId);

                try {
                    // Use notification client to check the email status
                    Notification notification = notificationClient.getNotificationById(emailState.notificationId);
