...
public interface LegalRepresentativeOrganisationRepository extends JpaRepository<LegalRepresentativeOrganisationEntity,
    ...
    @Query("""
        ...
        AND plro.active = 'YES'
        """)
    Optional<LegalRepresentativeOrganisationEntity> findByPartyLinkedToLegalRepresentativeOrganisationAndActive(
        @Param("partyId") UUID partyId);

    @Query("""
        SELECT lro
        FROM LegalRepresentativeOrganisationEntity lro
        JOIN lro.lro.claimPartyLegalRepresentativeOrganisationList plro
        JOIN plro.party p
