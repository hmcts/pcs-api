...
test.describe('[Review support request] - HearingCenterAdmin user - @nightly @CC @supportEvents', async () => {
  ...
  test('Approve the support request', async ({page}) => {
    ...
    await performAction('clickRadioButton', { option: 'Peter Parker (Defendant) - Special measure, Evidence by live link (Defendant Test Create Support)' });
    await performAction('clickButton', reviewSupport.continueButton);
    await performValidation('mainHeader', reviewSupport.reviewSupportHeader);
  ...
  test('Inactive the support request', async ({page}) => {
    ...
    await performValidation('mainHeader', reviewSupport.reviewSupportHeader);
    await performAction('inputText', reviewSupport.reviewCommentLabel, reviewSupport.reviewCommentText);
    await performAction('clickRadioButton', { question: reviewSupport.reviewStatusLabel, option: reviewSupport.inActiveOption });
    await performAction('clickButton', reviewSupport.continueButton);
    await performValidation('mainHeader', reviewSupport.reviewSupportHeader);
    await performAction('clickButton', reviewSupport.submitButton);
    await performValidation('bannerAlert', `Case #.* has been updated with event: Review support request`);
  ...
  test('Not approve the support request', async ({page}) => {
    ...
    await performValidation('mainHeader', reviewSupport.reviewSupportHeader);
    await performAction('inputText', reviewSupport.reviewCommentLabel, reviewSupport.reviewCommentText);
    await performAction('clickRadioButton', { question: reviewSupport.reviewStatusLabel, option: reviewSupport.notApproveOption });
    await performAction('inputText', {text:reviewSupport.reasonForStatusChangeHintText, index: null }, reviewSupport.reasonForStatusChangeText);
    await performAction('clickButton', reviewSupport.continueButton);
    await performValidation('mainHeader', reviewSupport.reviewSupportHeader);
    await performAction('clickButton', reviewSupport.submitButton);
    await performValidation('bannerAlert', `Case #.* has been updated with event: Review support request`);
  ...
  test('Approve the other type support request', async ({page}) => {
    await performAction('login', {email: user.claimantSolicitorForGATest.email, password: user.claimantSolicitorForGATest.password});
    await dismissCookieBanner(page, 'analytics');
    ...
    await performValidation('mainHeader', reviewSupport.mainHeader);
    await performAction('clickRadioButton', { option: reviewSupport.otherOption });
    await performAction('inputText', reviewSupport.otherSupportTypeLabel, reviewSupport.otherSupportTypeText);
    await performAction('clickButton', reviewSupport.continueButton);
    ...
    await performAction('clickButton', caseSummary.go);
    await performValidation('mainHeader', reviewSupport.reviewSupportHeader);
    await performAction('clickRadioButton', { option: 'Peter Parker (Defendant) - Special measure, Other Text (Defendant Test Create Support)' });
    await performAction('clickButton', reviewSupport.continueButton);
    await performValidation('mainHeader', reviewSupport.reviewSupportHeader);
    await performAction('inputText', reviewSupport.otherTypeReviewCommentLabel, reviewSupport.reviewCommentText);
    await performAction('clickRadioButton', { question: reviewSupport.otherTypeReviewStatusLabel, option: reviewSupport.activeOption });
    await performAction('clickButton', reviewSupport.continueButton);
    await performValidation('mainHeader', reviewSupport.reviewSupportHeader);
    await performAction('clickButton', reviewSupport.submitButton);
    await performValidation('bannerAlert', `Case #.* has been updated with event: Review support request`);
