...
test.describe('[Create Case] @Master @nightly', async () => {
  ...
  test('England - Assured tenancy with Rent arrears and other possession grounds', async ({ page }) => {
    await expect(page).toHaveScreenshot('claim-form-step-1.png', {
      mask: [
        page.locator('#claim-id-display'),
        page.locator('.dynamic-date-field'),
      ],
      fullPage: true,
      maxDiffPixels: 100
    });
    ...
    //await expect(page.locator('p.subtitle')).toHaveText(figmaPageContent.subtitle);
