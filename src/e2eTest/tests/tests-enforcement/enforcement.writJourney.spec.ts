...
import {
  ...
  fieldsMap,
  moneyMap,
} from "@utils/actions/custom-actions/custom-actions-enforcement/enforcement.action";
...
test.describe("[Enforcement - Writ of Possession]", async () => {
  test("Writ - Apply for a Writ of Possession - Have you hired HCEO [Yes] - Repayment [SOME] @PR @regression", async () => {
  ...
  test("Writ - Apply for a Writ of Possession - Claim sent to High Court [No] @PR @regression", async () => {
    ...
    await performValidation("errorMessage", {
      header: youCannotApplyForWrit.errors,
      message: youCannotApplyForWrit.errMessage,
    });
  });

  test(
    "Writ - Apply for a Writ of Possession [General application journey]",
    {
      annotation: {
        type: "issue",
        description:
          "General application journey is a placeholder for now,this test will be fully etched out when this is ready to be developed",
      },
    },
    async () => {
      await performAction("select", caseSummary.nextStepEventList, caseSummary.enforceTheOrderEvent);
      ...
      await performValidation("errorMessage", {
        header: youNeedPermission.eventCouldNotBeCreatedErrorMessage,
        message: youNeedPermission.errMessage,
      });
    },
  );
});
