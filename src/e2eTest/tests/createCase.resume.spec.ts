...
test.describe('[Create Case - Resume and Find case] @Master @nightly', async () => {
  test('England - Resume with saved options', async ({page},testInfo) => {
    try {
      ...
      console.warn(`⚠️ Non-critical test failed: ${testInfo.title}`);
      await testInfo.attach('Non-critical failure', {
        body: (error as Error).stack || (error as Error).message,
  ...
  test('England - Resume without saved options', async ({page},testInfo) => {
    try {
      await performAction('selectAddress', {
        postcode: addressDetails.englandCourtAssignedPostcode,
        addressIndex: addressDetails.addressIndex
