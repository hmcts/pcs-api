export const noticeOfYourIntention = {
  title: 'Create a case - HM Courts & Tribunals Service - GOV.UK',
  mainHeader: 'Notice of your intention to begin possession proceedings',
  continue: 'Continue',
  guidanceOnPosessionNoticePeriodsLink: 'guidance on possession notice periods (opens in a new tab)',
  mainHeaderNewTab: 'Understanding the possession action process: A guide for private landlords in England',
  judgementInlineText: 'A judge might not grant a possession order if you have not followed the correct notice procedure',
  servedNoticeInteractiveQuestion: 'Have you served notice to the defendants?',
  typeOfNotice: 'What type of notice did you serve?',
  typeOfNoticeInput: 'RWH20',
  yes: 'Yes',
  no: 'No'
}
