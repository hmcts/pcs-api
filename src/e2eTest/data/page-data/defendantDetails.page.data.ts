export const defendantDetails =
  {
    title: 'Create a case - HM Courts & Tribunals Service - GOV.UK',
    mainHeader: 'Defendant 1 details',
    doYouKnowTheDefendantName: 'Do you know the defendant\'s name?',
    defendantFirstName: 'Defendant\'s first name',
    firstNameInput: 'Test',
    defendantLastName: 'Defendant\'s last name',
    lastNameInput: 'User',
    firstName1Input: 'O\'Connor',
    lastName1Input: 'Smith-Jones',
    defendantCorrespondenceAddress: 'Do you know the defendants\' correspondence address?',
    isCorrespondenceAddressSame: 'Is the defendant\'s correspondence address the same as the address of the property you\'re claiming possession of?',
    additionalDefendants: 'Do you need to add another defendant?',
    yes: 'Yes',
    no: 'No',
    continue: 'Continue',
    addNew: 'Add new'
  };
