...
function captureDataForCYA(action: string, fieldName?: actionData | actionRecord, value?: actionData | actionRecord): void {
  if (action === 'changeCaseState' || action === 'enterApplicationDetails' || action === 'selectManageHearing' || action === 'cancelHearing'  || action === 'uploadADocument' || action === 'selectDocumentToAmend') {
