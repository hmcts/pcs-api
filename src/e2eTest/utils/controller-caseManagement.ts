...
function captureDataForCYA(action: string, fieldName?: actionData | actionRecord, value?: actionData | actionRecord): void {
  if (action === 'changeCaseState' || action === 'enterApplicationDetails' || action === 'uploadADocument' || action === 'selectDocumentToAmend' || action === 'selectManageParty' || action ==='addAHearing' || action === 'selectParty' || action === 'updatePartyDetails') {
