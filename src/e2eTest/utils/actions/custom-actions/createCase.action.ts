...
export class CreateCaseAction implements IAction {
  ...
  private async enterReasonForPossession(reasons: actionData) {
    ...
    for (let n = 0; n < reasons.length; n++) {
      ...
      // Passing `n` made inputText wait for an nth(n) that cannot exist — measured on PR-2655 as
      // 7 waits of 3s here, all of which then fell back to .first() anyway. Same field, no wait.
