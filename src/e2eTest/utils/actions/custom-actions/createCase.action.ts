...
export class CreateCaseAction implements IAction {
  ...
  private async provideDetailsOfRentArrears(rentArrears: actionData) {
    ...
    await performAction(
      "inputText",
      detailsOfRentArrears.totalRentArrearsLabel,
      rentArrearsData.rentArrearsAmountOnStatement
    );
    await performAction("clickRadioButton", {
    ...
    if (rentArrearsData.rentPaidByOthersOption == "Yes") {
      ...
      if (rentArrearsData.paymentOptions?.includes("Other")) {
        await performAction(
          "inputText",
          detailsOfRentArrears.paymentSourceLabel,
          detailsOfRentArrears.paymentOptionOtherInput
        );
      }
      await performAction("clickButton", "Continue");
    }
  ...
  private async createCaseAction(caseData: actionData): Promise<void> {
    ...
    process.env.SERVICE_AUTH_TOKEN = await new ServiceAuthUtils().retrieveToken({
      microservice: caseApiData.microservice,
    });
    process.env.IDAM_AUTH_TOKEN = (
      await Axios.create().post(accessTokenApiData.accessTokenApiEndPoint, accessTokenApiData.accessTokenApiPayload)
    ).data.access_token;
