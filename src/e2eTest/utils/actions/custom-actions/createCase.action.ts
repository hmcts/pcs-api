...
export class CreateCaseAction implements IAction {
  ...
  private async selectEventAndGo(page: Page, event: actionRecord) {
    ...
    // Record the event-trigger requests each Go click produces. A real failure (PR-2692 build 2)
    // showed the dropdown holding the right value on all five attempts with no error anywhere, so
    // the open question is whether the click reaches CCD at all.
