...
export class SearchCaseAction implements IAction {
  ...
  private async searchMyCaseFromFindCase(page: Page, selectCriteriaCaseNumber: actionRecord) {
    ...
    if (searchCondition.criteria) {
      ...
      await page.waitForURL(`${process.env.MANAGE_CASE_BASE_URL}/cases/case-details/${process.env.CHANGE_ID ? `PCS-${process.env.CHANGE_ID}` : 'PCS'}/PCS/${searchCondition.caseNumber.replaceAll('-', '')}#Summary`);
