...
export class CaseManagementAction implements IAction {
  ...
  private async addAHearing(addAHearing: actionRecord) {
    ...
    // happy-path helper, on top of the 46 the error-validation path spent (measured on PR-2655).
      ...
      for (const item of validationArr.inputArray) {
        ...
        switch (validationArr.validationType) {
          ...
          case 'moneyField':
            ...
            // nth(1) can never attach: PR-2655 measured 46 waits of 3s here, every one of which
            // then fell back to .first(). Dropping the index reaches the same field without the
            // wait, and leaves which fields get filled untouched.
